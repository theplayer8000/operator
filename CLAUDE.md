...
| **Brave Search** | `api.search.brave.com` | **2026-09-04**, `server/websearch.mjs`. Approved by name so any worker can look something up — Claude Code has WebSearch inside its own harness and AI Router, Gemini and Ollama had no web access at all, so Operator could not know anything past a training cutoff. What leaves the machine: **the search query, and nothing else** — no prompt, no job context, no file. **A query is a sharper disclosure than a prompt**, which is why this needed its own row rather than riding on the AI Router approval: queries are short, specific, and state intent more plainly than the paragraph around them. Brave claims Zero Data Retention and a first-party index; that is a policy, not a structural property, same caveat as the router. Key in `BRAVE_SEARCH_API_KEY`, env only, set with `secret_set`. Results are cached briefly and a daily request ceiling applies (`OPERATOR_SEARCH_DAILY_MAX`) because this one is metered per request — roughly $5 per 1,000 |
| **Runway** | `api.dev.runwayml.com` | **2026-09-04**, `server/runway.mjs`. Video generation, asked for by name. What leaves the machine: **a text prompt, and for image-to-video an image the owner supplies**. That image is the reason this is a separate row and not a variation of the others — a picture is a different category of disclosure from text he typed, the same line ADR 0016 draws around audio. Only images passed to the action are sent; nothing reads his disk to find one. **Asynchronous by nature** — a generation takes minutes — so it is two actions (start, then check) rather than a worker turn that blocks. Key in `RUNWAY_API_KEY`, env only, set with `secret_set`. **The endpoint and API version are unverified against Runway's live documentation** and are env-overridable for exactly that reason; the first real call is the test |
| **Roblox Open Cloud** | `apis.roblox.com` | **2026-09-04**, `server/roblox.mjs`. Asked for by name — he wanted to *toy with the API*, so this is a deliberate **passthrough** to any Open Cloud endpoint rather than one action per question. What leaves the machine: **whatever path, query and body the caller passes**, plus the key as an `x-api-key` header attached server-side. That is a wider disclosure than every other row here, and it is bounded by three walls rather than by the shape of one endpoint: the host is fixed and the path must start with `/cloud/`, so the key cannot be aimed elsewhere; headers are not caller-settable, so a worker can use the key without learning it; and the method is one of GET/POST/PATCH/PUT/DELETE. **The mutating verbs are deliberate** — he ticked every scope when creating the key — which makes this the one approved host where a worker can change state on a service he does not own. **Not a generic HTTP gateway, and must not grow into one**: an any-URL action is a separate approval conversation, not a wider door here. Key in `OPEN_CLOUD_API_KEY`, env only, set with `secret_set`. **The header name and base host come from Roblox's docs, not from a call that succeeded**; the first real call is the test |
...
- **Set one with the `secret_set` action, never by typing `setx` anywhere.**
  The terminal logs every command it runs — that audit line is the point of ADR
  0011 — and `setx SOME_KEY <value>` there wrote the secret to the server log in
  plaintext. The Gemini key had to be reissued for exactly this.
  `secret_set` exists so the ergonomics stop fighting the rule: it takes a name
  and a value, and the value reaches no log, no event, no response body and not
  the store. It **refuses any `OPERATOR_*` name** — that namespace is the
  security boundary, not configuration, and a device able to write
  `OPERATOR_TERMINAL_DEVICES` could grant itself execution. Those stay a
  desk job. Typing `setx` in the terminal now redacts the value in the audit
  line as a safety net, but the action is the route.
...
server/
  ...
  websearch.mjs         — one search, through Brave. The only way a worker
                          without its own harness can know anything after a
                          training cutoff. Sends the QUERY and nothing else —
                          not the prompt, not the job, not a file. Cached
                          briefly and capped per day, because unlike the router
                          this one is metered per request
  runway.mjs            — video generation. Two actions rather than a turn: a
                          generation takes minutes, so one starts it and one
                          asks whether it is done. Sends a prompt, and an image
...
| Gym | `/gym` | Built — today's session as a tickable checklist, day stepper, rest-day and skipped states. Five sessions named by push/pull structure, keyed by ISO weekday. Ticks are stored per date (`gym.completions`), skipped days separately (`gym.skipped`). The programme itself — phases, percentages, deloads, nutrition — is owner content in `reference/gym-programme.md`, not `/docs`. **The gym mission tracks training adherence** (2026-09-10): `gym_log_session` logs a whole session AND moves the gym mission's slider to month-to-date adherence — sessions trained / sessions scheduled since the 1st, both **derived** (from `gym.sessions` × the calendar, and `gym.completions`), nothing stored twice. Targets the `category:"gym"` mission whose name matches `/week\|days/i`. "Went gym / did push day" (intent.mjs) emits it; the Gym-page path gets a read-only adherence line + Apply on the mission detail (`GymAdherence.tsx`) since features stay write-independent. `useGym().monthAdherence()` is the shared derivation |
...
| Knowledge Vault | `/knowledge`, `/knowledge/:id` | Built 2026-09-03 — the personal wiki the Mission Board has pointed at since it was built. Notes carry a **kind** (note/command/resource) and a **confidence** (unverified/worked/verified), and confidence is the field that earns its place: a vault's failure mode is something written once while learning and trusted a year later as though it were checked. Links are **directional ids, never `[[wiki syntax]]` in the body** — prose-embedded links break on rename and nothing can tell you; backlinks are derived by filtering, exactly like mission successors. Search goes through `lib/search.ts`, which **is the Search Service boundary ADR 0009 approved by name** — a ranked word scan today, an embedding lookup later, and one file changes. **Seven capability actions**, and the reads matter more than the writes here than anywhere: a vault a worker cannot query is a folder. Archive only — there is deliberately no `knowledge_delete`. Two routes, one page: the id only decides what is open |
| Decision Log | `/decisions`, `/decisions/:id` | Built 2026-09-10 — the *life* decision log (took the job, sold the car), **not** the engineering ADRs in `docs/decisions/`. Fields: decision / `decidedOn` / reasoning / outcome, plus a `verdict` (pending/good/mixed/bad) and topics. **Outcome and verdict start empty** — the gap between a decision and its consequence is real and the UI shows it. Built as a near-exact clone of the Knowledge Vault: one namespace (`decisions.records`), one hook, one page, two routes, archive-only (no `decision_delete`). Five capability actions (`decision_log`/`decision_get` are reads and silent; `decision_add`/`decision_update`/`decision_archive` notify). Search is a plain word scan in the hook — **`lib/search.ts` was deliberately not extended** to a second entity type (ADR 0009). Wired into `MissionDetail`'s Related Decisions tab (read-only; attach from the decision). |
